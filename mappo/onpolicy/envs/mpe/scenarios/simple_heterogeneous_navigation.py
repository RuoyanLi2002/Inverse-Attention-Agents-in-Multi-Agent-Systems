...
class Scenario(BaseScenario):
    def make_world(self, args):
        ...
        print(f"Wolf(Big and Slow): {self.num_adversaries}; Sheep(Small and Fast): {self.num_good_agents}; Landmark: {args.num_landmarks}; Use Shape Reward: {self.use_shape_reward}")
        # add agents
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.size = 0.1 if agent.adversary else 0.06
            agent.accel = 2.0 if agent.adversary else 4.0
            agent.max_speed = 1 if agent.adversary else 2
        ...
        world.past_action = [np.zeros(5).astype(np.float32) for _ in range(self.num_agents)]
    ...
    def reward(self, agent, world):
        ...
        for temp_agent in world.agents:
            for landmark in world.landmarks:
                if self.is_collision(temp_agent, landmark):
                    total_reward += 5

            if self.use_shape_reward:
                dis_reward = min([self.distance_between_agents(temp_agent, landmark) for landmark in world.landmarks])
    ...
    def obs_GFATTENTION(self, agent, world):
        ...
        agent_pos = np.concatenate([agent.state.p_pos, np.array([1.0])])
        ...
        for other in world.agents:
            if other is agent:
                ...
                other_pos = np.concatenate([other.state.p_pos, np.array([1.0])])
