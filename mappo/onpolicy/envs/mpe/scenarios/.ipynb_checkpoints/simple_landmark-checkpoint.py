...
class Scenario(BaseScenario):
    def make_world(self, args):
        ...
        world.ls_policy = ["GF+ATTENTION", "GF+ATTENTION", "TOM", "GF+ATTENTION"]
    ...
    def reward(self, agent, world):
        ...
        if self.use_shape_reward:
            dis_reward = min([self.distance_between_agents(agent, landmark) for landmark in world.landmarks])
    ...
    def obs_GFATTENTION(self, agent, world):
        ...
        for landmark in world.landmarks:
            landmark_pos = np.concatenate([landmark.state.p_pos, np.array([1.0])])
