...
class Scenario(BaseScenario):
    def make_world(self, args):
        ...
        print(f"Wolf: {self.num_adversaries}; Sheep: {self.num_good_agents}; Use Shape Reward: {self.use_shape_reward}")
        # add agents
        ...
        world.past_obs = [[np.random.standard_normal(2).tolist() for i in range(self.num_agents)] for j in range(self.num_agents)]
        world.new_obs = [[] for _ in range(self.num_agents)]

        # world.ls_policy = ["GF+ATTENTION" for i in range(self.num_agents)]
        # world.ls_policy = ["GF+ATTENTION", "GF+ATTENTION", "GF+ATTENTION", "GF+ATTENTION", "GF+ATTENTION", "TOM", "GF+ATTENTION", "GF+ATTENTION"]
    ...
    def reward_sheep(self, agent, world, penalty = -100):
        total_reward = 0
        '''
        for temp_agent in world.agents:
            if temp_agent is agent:
                ...
                total_reward += self.distance_between_agents(agent, temp_agent)
        '''
        for temp_agent in world.agents:
            if temp_agent is agent:
                continue
    ...
    def obs_MAPPO(self, agent, world):
        ...
        # return np.concatenate([agent.state.p_pos]+[agent.state.p_vel] + other_pos + other_vel)
        ...
        return np.concatenate([agent.state.p_pos]+[agent.state.p_vel] + other_pos)
